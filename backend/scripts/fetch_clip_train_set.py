...
KERNEL01_TARGET_EACH = 150

CF_TRAIN_OFFSETS = list(range(35000, 51500, 1500))  # stays within the 51,836-row split
CF_TRAIN_TARGET_EACH = 80
...
def fetch_kernel01(manifest: list[tuple[str, str, str]], seen_hashes: set[str]) -> None:
    ...
    while (real_count < KERNEL01_TARGET_EACH or ai_count < KERNEL01_TARGET_EACH) and offset < 8000:
        url = (
...
def fetch_community_forensics(manifest: list[tuple[str, str, str]], seen_hashes: set[str]) -> None:
    ...
    for offset in CF_TRAIN_OFFSETS:
        if real_count >= CF_TRAIN_TARGET_EACH and ai_count >= CF_TRAIN_TARGET_EACH:
            break
        url = (
        ...
        for row in d.get("rows", []):
            ...
            if label == 0 and real_count < CF_TRAIN_TARGET_EACH:
                ...
                if gate_result.status == "ok":
                    ...
                    real_kept += 1
            elif label == 1 and ai_count < CF_TRAIN_TARGET_EACH:
                gate_result = gate.run(img)
                ai_count += 1
                if gate_result.status == "ok":
                    seen_hashes.add(h)
